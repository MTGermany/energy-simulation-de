...
const windOn_av=0.905;
const windOff_av=0.85; 
const curtailfactor_windOff=0.6; // grid curtailment to factor*pow00

const minPow_rotatingMass=8;

const loadSheddingFactor=0.3; // to prevent blackout, shed load up to factor
...
console.log("frac_onshore[1]=",frac_onshore[1]);



// solar yearly performance characteristic (efficiency higher if cold)
// pWind=p0*I/Iref*(1-amplRel*Math.cos(phi_year-dphi)
...
simulation.prototype.update=function(it){
  ...
  var minNuclear=nuclear_minRelPow*nuclear_av*pow0_Nuclear;
...
//		  " no valid solar data for region ",i," using valid avg");
  ...
  let r=(maxWindOff>0)
      ? Math.min(maxWindOff, curtailfactor_windOff* pow0_WindOff)/maxWindOff
      : 1;
  maxWindOff *=r;
  this.hourlyWindRegions.region4 *=r;
  this.hourlyWindRegions.region5 *=r;
